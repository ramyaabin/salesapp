...
export default function App() {
  ...
  useEfimport { useEffect, useState } from "react";
import hamaLogo from "./assets/Hama.jpeg";
import api from "./api";
import {
  PieChart,
...
/* ===================== LOADING SPINNER ===================== */
// ✅ Leave status badge with colour coding
              ...
              {filteredProducts.length > 0 ? (
                filteredProducts.map((p, i) => (
                  <div key={i} className="prod-row">
                    <div style={{ flex: 1 }}>
                      <div className="prod-row-brand">{p.brand}</div>
                      <div className="prod-row-code">{p.modelNumber || "—"}</div>
                      <div className="prod-row-code" style={{ fontSize: 11 }}>🔖 {p.ean || "—"}</div>
                      <div className="prod-row-code" style={{ fontSize: 11 }}>{p.description || ""}</div>
                    </div>
                    <div className="prod-row-price">{money(p.price)}</div>
                  </div>
                ))
                  ...
                  {filteredProducts.length > 0 ? (
                    filteredProducts.map((p, i) => (
                      <tr key={i}>
                        ...
                        <td style={styles.td}>{p.modelNumber || "—"}</td>
                        <td style={styles.td}>{p.ean || "—"}</td>
                        <td style={{ ...styles.td, maxWidth: 300, whiteSpace: "normal" }}>{p.description || "—"}</td>
                        <td style={styles.td}>{money(p.price)}</td>
                      </tr>
                    ))
                  ) : (
                ...
                <tbody>
                  {dailyLeaves.map((leave, idx) => (
                    <tr key={idx}>
                      <td style={styles.td}>{formatDate(leave.date)}</td>
                      <td style={styles.td}>{leave.reason}</td>
                      <td style={styles.td}><LeaveStatusBadge status={leave.status} /></td>
                      <td style={styles.td}>{leave.adminRemark || "—"}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
const AdminDashboard = ({ user, navigate, onLogout }) => {
  ...
  const downloadLeaveReport = (salesmanId) => {
    ...
    let csv = "Date,Salesman,Reason,Applied At,Status,Admin Remark,Action At\n";
    salesmanLeaves.forEach((l) => {
      const reason = `"${(l.reason || "").replace(/"/g, '""')}"`;
      const remark = `"${(l.adminRemark || "").replace(/"/g, '""')}"`;
      const appliedAt = l.appliedAt ? new Date(l.appliedAt).toLocaleString("en-AE") : "";
      const actionAt = l.actionAt ? new Date(l.actionAt).toLocaleString("en-AE") : "";
      csv += `${l.date},${salesman?.name || l.salesmanName},${reason},${appliedAt},${l.status || "Pending"},${remark},${actionAt}\n`;
    });
    const blob = new Blob([csv], { type: "text/csv" });
    const url = window.URL.createObjectURL(blob);
    const a = document.createElement("a");
  ...
  // ✅ Approve or Reject leave with admin remark
  const handleLeaveAction = async (status) => {
    if (!remarkModal) return;
    try {
      await api.updateLeaveStatus(remarkModal.leave._id, status, adminRemark);
      await refreshLeaves();
      setRemarkModal(null);
      setAdminRemark("");
    } catch (err) {
    ...
    <>
              ...
              <table style={styles.table}>
                ...
                <tbody>
                  {dailyLeaves.map((leave, idx) => {
                    ...
                    return (
                      <tr key={idx}>
                        <td style={styles.td}>{salesman?.name || leave.salesmanName}</td>
                        <td style={styles.td}>{leave.reason}</td>
                        <td style={styles.td}>{leave.appliedAt ? new Date(leave.appliedAt).toLocaleString("en-AE") : formatDate(leave.date)}</td>
                        <td style={styles.td}>
                        ...
                        <td style={styles.td}>
                          {leave.status === "Pending" && (
                            <div style={{ display: "flex", gap: 6 }}>
                              <button onClick={() => { setRemarkModal({ leave }); setAdminRemark(""); }} style={{ ...styles.actionButton, background: "#4caf50" }}>✓ Approve</button>
                              <button onClick={() => { setRemarkModal({ leave, requireRemark: true }); setAdminRemark(""); }} style={{ ...styles.actionButton, background: "#f44336" }}>✗ Reject</button>
                            </div>
                          )}
                          {leave.status !== "Pending" && leave.adminRemark && (
                            <span style={{ fontSize: 12, color: "#666" }}>📝 {leave.adminRemark}</span>
                          )}
                        </td>
                      </tr>
                    );
          ...
          {view === "monthly" && monthlyLeaves.length > 0 && (
            <div className="card" style={styles.card}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                ...
                <button
                  onClick={downloadAllLeavesReport}
                  style={{ ...styles.button, width: "auto", padding: "8px 16px", fontSize: 13 }}
                >
                  ⬇ Download All Leaves CSV
                </button>
              </div>
                ...
                <table style={styles.table}>
                  <thead>
                    <tr>
                      ...
                      <th style={styles.th}>Status</th>
                      <th style={styles.th}>Admin Remark</th>
                      <th style={styles.th}>Action At</th>
                      <th style={styles.th}>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {monthlyLeaves.map((leave, idx) => {
                      ...
                      return (
                        <tr key={idx}>
                          ...
                          <td style={styles.td}>{salesman?.name || leave.salesmanName}</td>
                          <td style={styles.td}>{leave.reason}</td>
                          <td style={styles.td}>{leave.appliedAt ? new Date(leave.appliedAt).toLocaleString("en-AE") : "—"}</td>
                          <td style={styles.td}><LeaveStatusBadge status={leave.status} /></td>
                          <td style={styles.td}>{leave.adminRemark || "—"}</td>
                          <td style={styles.td}>{leave.actionAt ? new Date(leave.actionAt).toLocaleString("en-AE") : "—"}</td>
                          <td style={styles.td}>
                            {leave.status === "Pending" && (
                              <div style={{ display: "flex", gap: 6 }}>
                                <button onClick={() => { setRemarkModal({ leave }); setAdminRemark(""); }} style={{ ...styles.actionButton, background: "#4caf50" }}>✓</button>
                                <button onClick={() => { setRemarkModal({ leave, requireRemark: true }); setAdminRemark(""); }} style={{ ...styles.actionButton, background: "#f44336" }}>✗</button>
                              </div>
                            )}
                          </td>
                        </tr>
      ...
      {remarkModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div style={{ background: "#fff", borderRadius: 12, padding: 28, width: 420, maxWidth: "90vw", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}>
            <h3 style={{ margin: "0 0 8px", color: remarkModal.requireRemark ? "#c62828" : "#2e7d32" }}>
              {remarkModal.requireRemark ? "✗ Reject Leave" : "✓ Approve Leave"}
            </h3>
            <p style={{ margin: "0 0 16px", color: "#555", fontSize: 14 }}>
              <strong>{remarkModal.leave.salesmanName}</strong> — {formatDate(remarkModal.leave.date)}<br />
              Reason: {remarkModal.leave.reason}
            </p>
            <label style={{ fontSize: 13, fontWeight: 600, color: "#333" }}>
              {remarkModal.requireRemark ? "Rejection Reason *" : "Note (optional)"}
            </label>
            <textarea
              value={adminRemark}
              onChange={(e) => setAdminRemark(e.target.value)}
              placeholder={remarkModal.requireRemark ? "Enter reason for rejection..." : "Add a note (optional)..."}
              rows={3}
              style={{ width: "100%", marginTop: 6, padding: "8px 10px", borderRadius: 6, border: "1px solid #ddd", fontSize: 14, boxSizing: "border-box", resize: "vertical" }}
            />
            <div style={{ display: "flex", gap: 10, marginTop: 16, justifyContent: "flex-end" }}>
              <button onClick={() => { setRemarkModal(null); setAdminRemark(""); }} style={{ ...styles.headerButton, padding: "8px 18px" }}>Cancel</button>
              <button
                onClick={() => {
                  if (remarkModal.requireRemark && !adminRemark.trim()) {
                    alert("Please enter a rejection reason.");
                    return;
                  }
                  handleLeaveAction(remarkModal.requireRemark ? "Rejected" : "Approved");
                }}
                style={{ ...styles.button, width: "auto", padding: "8px 20px", background: remarkModal.requireRemark ? "#f44336" : "#4caf50" }}
              >
                {remarkModal.requireRemark ? "Reject" : "Approve"}
              </button>
            </div>
          </div>
        </div>
